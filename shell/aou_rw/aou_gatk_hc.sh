...
cat << EOF > $staging_dir/ExcludeSnvOutliersFromSvCallset.inputs.template.json
{
  ...
  "PosthocHardFilterPart2.vcf": "$MAIN_WORKSPACE_BUCKET/dfci-g2c-callsets/gatk-sv/module-outputs/CollapseRedundantSvs/\$CONTIG/RC3/dfci-g2c.v1.\$CONTIG.concordance.gq_recalibrated.gq_updated.identical.reclustered.vcf.gz",
  "PosthocHardFilterPart2.vcf_idx": "$MAIN_WORKSPACE_BUCKET/dfci-g2c-callsets/gatk-sv/module-outputs/CollapseRedundantSvs/\$CONTIG/RC3/dfci-g2c.v1.\$CONTIG.concordance.gq_recalibrated.gq_updated.identical.reclustered.vcf.gz.tbi"
}
EOF
...
##########################################################
# Collect GATK-SV QC metrics after SNV outlier exclusion #
##########################################################
...
# Reaffirm staging directory
staging_dir=staging/snv_outlier_excluded_gatksv_qc
if ! [ -e $staging_dir ]; then mkdir $staging_dir; fi

# Write template input .json for QC metric collection
cat << EOF > $staging_dir/CollectSnvOutlierExcludedGatksvQcMetrics.inputs.template.json
{
  ...
  "CollectVcfQcMetrics.output_prefix": "dfci-g2c.v1.snv_outlier_excluded_gatksv_qc.\$CONTIG",
  ...
  "CollectVcfQcMetrics.vcfs_array": ["$MAIN_WORKSPACE_BUCKET/dfci-g2c-callsets/gatk-sv/module-outputs/ExcludeSnvOutliersFromSvCallset/\$CONTIG/HardFilterPart2/dfci-g2c.v1.\$CONTIG.concordance.gq_recalibrated.gq_updated.identical.reclustered.posthoc_filtered.vcf.gz"],
  "CollectVcfQcMetrics.vcf_idxs_array": ["$MAIN_WORKSPACE_BUCKET/dfci-g2c-callsets/gatk-sv/module-outputs/ExcludeSnvOutliersFromSvCallset/\$CONTIG/HardFilterPart2/dfci-g2c.v1.\$CONTIG.concordance.gq_recalibrated.gq_updated.identical.reclustered.posthoc_filtered.vcf.gz.tbi"]
}
EOF
...
code/scripts/manage_chromshards.py \
  ...
  --input-json-template $staging_dir/CollectSnvOutlierExcludedGatksvQcMetrics.inputs.template.json \
  --dependencies-zip qc.dependencies.zip \
  --staging-bucket $MAIN_WORKSPACE_BUCKET/dfci-g2c-callsets/qc-filtering/snv-outlier-excluded-qc/GatksvQcMetrics/ \
  --name CollectSnvOutlierExcludedGatksvQcMetrics \
  --contig-list contig_lists/dfci-g2c.v1.contigs.$WN.list \
  --status-tsv cromshell/progress/dfci-g2c.v1.CollectSnvOutlierExcludedGatksvQcMetrics.progress.tsv \
  --workflow-id-log-prefix "dfci-g2c.v1" \
  --outer-gate 30 \
  --submission-gate 3 \
  --max-attempts 3
...
######################################################################
# Analyze & visualize GATK-SV QC metrics after SNV outlier exclusion #
######################################################################
...
for k in $( seq 1 22 ) X Y; do
  ...
  json_fname=CollectSnvOutlierExcludedGatksvQcMetrics.chr$k.outputs.json
  gsutil cp \
    $MAIN_WORKSPACE_BUCKET/dfci-g2c-callsets/qc-filtering/snv-outlier-excluded-qc/GatksvQcMetrics/chr$k/$json_fname \
    $staging_dir/
  while read key; do
...
# Write input .json
cat << EOF | python -m json.tool > cromshell/inputs/PlotSnvOutlierExcludedGatksvQcMetrics.inputs.json
{
  ...
  "PlotVcfQcMetrics.output_prefix": "dfci-g2c.v1.snv_outlier_excluded_gatksv_qc",
...
cromshell --no_turtle -t 120 -mc submit --no-validation \
  ...
  cromshell/inputs/PlotSnvOutlierExcludedGatksvQcMetrics.inputs.json \
| jq .id | tr -d '"' \
>> cromshell/job_ids/dfci-g2c.v1.PlotSnvOutlierExcludedGatksvQcMetrics.job_ids.list

# Monitor QC visualization workflow
monitor_workflow $( tail -n1 cromshell/job_ids/dfci-g2c.v1.PlotSnvOutlierExcludedGatksvQcMetrics.job_ids.list ) 5
...
cromshell -t 120 list-outputs \
  $( tail -n1 cromshell/job_ids/dfci-g2c.v1.PlotSnvOutlierExcludedGatksvQcMetrics.job_ids.list ) \
| awk '{ print $2 }' \
| gsutil -m cp -I \
...
gsutil -m ls $( cat cromshell/job_ids/dfci-g2c.v1.PlotSnvOutlierExcludedGatksvQcMetrics.job_ids.list \
...
# Reaffirm staging directory
staging_dir=staging/snv_outlier_excluded_dev_gatksv_qc
if ! [ -e $staging_dir ]; then mkdir $staging_dir; fi
...
while read contig; do
  ...
  gsutil cp \
    $MAIN_WORKSPACE_BUCKET/dfci-g2c-callsets/qc-filtering/snv-outlier-excluded-qc/GatksvQcMetrics/$contig/$json_fname \
    $staging_dir/
  while read key; do
    jq .\"CollectVcfQcMetrics.$key\" $staging_dir/$json_fname \
    | fgrep -xv "null" | tr -d '"' \
  ...
  # Add precomputed gnomAD v4.1 reference distributions to file lists
  for suffix in af_distribution size_distribution; do
    echo "gs://dfci-g2c-refs/gnomad/gnomad_v4_site_metrics/$contig/gnomad.v4.1.gatksv.$contig.$suffix.merged.tsv.gz" \
    >> $staging_dir/gnomAD_$suffix.uris.list
  done
done < ~/contig_lists/dfci-g2c.v1.contigs.dev.list

# Write input .json
cat << EOF | python -m json.tool > cromshell/inputs/PlotSnvOutlierExcludedDevGatksvQcMetrics.inputs.json
{
  "PlotVcfQcMetrics.af_distribution_tsvs": $( collapse_txt $staging_dir/af_distrib.uris.list ),
  "PlotVcfQcMetrics.all_sv_beds": $( collapse_txt $staging_dir/all_svs_bed.uris.list ),
  "PlotVcfQcMetrics.bcftools_docker": "us.gcr.io/broad-dsde-methods/gatk-sv/sv-base-mini:2024-10-25-v0.29-beta-5ea22a52",
  ...
  "PlotVcfQcMetrics.deduplicate": true,
  "PlotVcfQcMetrics.g2c_analysis_docker": "vanallenlab/g2c_analysis:ed9676d",
  "PlotVcfQcMetrics.output_prefix": "dfci-g2c.v1.snv_outlier_excluded_dev_gatksv_qc",
  "PlotVcfQcMetrics.peak_ld_stat_tsvs": $( collapse_txt $staging_dir/ld_stats.uris.list ),
  "PlotVcfQcMetrics.PlotSiteBenchmarking.mem_gb": 32,
  "PlotVcfQcMetrics.PlotSiteBenchmarking.n_cpu": 8,
  "PlotVcfQcMetrics.PlotSiteMetrics.mem_gb": 32,
...
cromshell --no_turtle -t 120 -mc submit --no-validation \
  ...
  --dependencies-zip qc.dependencies.zip \
  code/wdl/pancan_germline_wgs/vcf-qc/PlotVcfQcMetrics.wdl \
  cromshell/inputs/PlotSnvOutlierExcludedDevGatksvQcMetrics.inputs.json \
| jq .id | tr -d '"' \
>> cromshell/job_ids/dfci-g2c.v1.PlotSnvOutlierExcludedDevGatksvQcMetrics.job_ids.list
