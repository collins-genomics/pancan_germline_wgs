...
def hwe_dist(x, signed=True):
    """
    Computes the Euclidean distance between a variant's observed genotype counts
    and expected genotype counts under Hardy-Weinberg equilibrium

    `x` : either a pysam.VariantRecord or a dict generated by g2cpy.compute_allele_freq_stats()

    Assumes biallelic variants at diploid loci

    Returns a tuple of (hwe distance, excess hets, excess hom)

    If `signed` is `True`, will return a positive value when homozygotes are overcalled
    and a negative value when they are undercalled
    """

    if isinstance(x, VariantRecord):
        x = g2cpy.compute_allele_freq_stats(x, keys='AF N_REF N_HET N_HOM'.split())

    obs = [int(x[k]) for k in 'N_REF N_HET N_HOM'.split()]
    p = x['AF']
    q = 1 - p
    n = sum(obs)
    exp_ref = n * (q**2)
    exp_het = 2 * n * p * q
    exp_hom = n * (p**2)
    exp = [exp_ref, exp_het, exp_hom]
    ex_het = obs[1] - exp_het
    ex_hom = obs[2] - exp_hom

    # Return 
    if ex_hom >= 0:
        return tuple([dist(obs, exp), ex_het, ex_hom])
    else:
        return tuple([-dist(obs, exp), ex_het, ex_hom])
