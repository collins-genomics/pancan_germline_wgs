...
workflow GatherSampleEvidence {
  input {
    ...
    String? manta_machine
    RuntimeAttr? runtime_attr_melt_coverage
    String? melt_coverage_machine
    RuntimeAttr? runtime_attr_melt_metrics
    String? melt_metrics_machine
  ...
  if (run_melt) {
    call melt.MELT {
      input:
        ...
        coverage_machine_type = select_first([melt_coverage_machine, "e2-standard-2"]),
        runtime_attr_metrics = runtime_attr_melt_metrics,
        metrics_machine_type = select_first([melt_metrics_machine, "e2-standard-2"]),
        ...
        melt_machine_type = select_first([melt_machine, "e2-standard-2"])
    }
  }
