# Germline VCF Quality Control and Visualization Pipeline

This directory contains a modular two-step pipeline for **comprehensive quality control (QC) and visualization of germline variation callsets**. The pipeline is designed to evaluate the quality, consistency, and basic characteristics of **joint-genotyped VCFs**.

This pipeline has **only been tested on VCFs generated by GATK-HC and/or GATK-SV from short-read whole-genome sequencing (WGS)**. We anticipate it should easily transfer to other technologies (e.g., exomes, long-read WGS) or variant calling methods (e.g., DeepVariant) as long as they are jointly genotyped, but this compatability **has not yet been tested**.
...
2. [`PlotVcfQcMetrics.wdl`](./PlotVcfQcMetrics.wdl) — merges, summarizes, and visualizes QC results across all chromosomes.
...
The QC pipeline provides a standardized framework to assess key properties of a germline VCF, including:
...
- Aggregated plots and tables summarizing genome-wide quality metrics
...
|------|-----------|-------------|
| **1. Collect per-chromosome QC metrics** | [`CollectVcfQcMetrics.wdl`](./CollectVcfQcMetrics.wdl) | Gathers QC metrics for one or more VCFs, optionally scattered by chromosome or interval. |
| **2. Merge and visualize results** | [`PlotVcfQcMetrics.wdl`](./PlotVcfQcMetrics.wdl) | Combines metrics from all chromosomes and produces summary tables and plots for cohort-level interpretation. |

---

## 1️⃣ CollectVcfQcMetrics.wdl

### Description

`CollectVcfQcMetrics.wdl` runs QC analyses on one or more input VCFs (SNVs/indels and/or SVs).  
It can scatter across chromosomes or intervals to parallelize processing, then aggregate outputs into per-shard metric tables.  

### Key Inputs

| Input | Type | Required | Description |
|--------|------|-----------|-------------|
| `vcfs` | `Array[File]` | ✅ | GATK joint-genotyped VCF(s) for analysis. |
| `vcf_idxs` | `Array[File]` | ✅ | Corresponding VCF index files (`.tbi`). |
| `shard_vcf` | `Boolean` | ❌ (default: `true`) | Whether to split input VCF for scatter-based QC collection. |
...
## 2️⃣ PlotVcfQcMetrics.wdl

### Description

`PlotVcfQcMetrics.wdl` aggregates per-chromosome QC metrics into unified cohort-level plots and tables.  
It can optionally overlay reference datasets to contextualize QC profiles.
